  ...
  try {
    ...
    try {
      for (const content of contents) {
        ...
        if (!title || !type) {
          return NextResponse.json(
            {
              status: "error",
              error: "Her içerik için başlık ve içerik türü zorunludur.",
            },
            { status: 400 }
          );
        }
        ...
        if (type === "VIDEO" || type === "DOCUMENT") {
          if (!fileUrl) {
            return NextResponse.json(
              {
                status: "error",
                error: "Video veya doküman için dosya URL'si zorunludur.",
              },
              { status: 400 }
            );
          }
          ...
          if (!gameUrl) {
            return NextResponse.json(
              {
                status: "error",
                error: "Oyun eklemek için oyun URL'si zorunludur.",
              },
              { status: 400 }
            );
          }
      ...
      return NextResponse.json(
        {
          status: "error",
          error: "İçerik doğrulama sırasında bir hata oluştu.",
        },
        { status: 400 }
      );
    }
    ...
    try {
      formattedContents = contents.map((content) => {
        let studentDate = content.publishDateForStudent
          ? new Date(content.publishDateForStudent)
          : null;
      ...
      return NextResponse.json(
        {
          status: "error",
          error: "Yayınlanma tarihleri belirlenirken bir hata oluştu.",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          status: "error",
          error: error.message || "İçerikler eklenirken bir hata oluştu.",
        },
        { status: 500 }
      );
    }
    ...
    if (!newContents || newContents.error) {
      return NextResponse.json(
        {
          status: "error",
          error: newContents.error || "İçerikler eklenirken bir hata oluştu.",
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      {
        status: "success",
        message:
          formattedContents.length === 1
            ? "İçerik başarıyla eklendi."
            : "İçerikler başarıyla eklendi.",
